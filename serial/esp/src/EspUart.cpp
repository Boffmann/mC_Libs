...
EspUart::EspUart(const std::string portName, const int portNumber,
                 const int txPin, const int rxPin)
    : portName(portName), portNumber(portNumber), txPin(txPin), rxPin(rxPin),
      initialized(false), baudRate(115200) {}
...
bool EspUart::initialize(const SerialConfig &config, const uint32_t baudRate) {
  ...
  esp_err_t ret = uart_driver_install(this->portNumber, 1024, 1024, 10,
                                      &this->uart_queue, 0);
  if (ret != ESP_OK) {
  ...
  uartConfig_esp.flow_ctrl =
      config.flowControl ? UART_HW_FLOWCTRL_CTS_RTS : UART_HW_FLOWCTRL_DISABLE;
  ...
  if (txPin >= 0 && rxPin >= 0) {
    ret = uart_set_pin(this->portNumber, txPin, rxPin, UART_PIN_NO_CHANGE,
                       UART_PIN_NO_CHANGE);
    if (ret != ESP_OK) {
      uart_driver_delete(this->portNumber);
...
bool EspUart::isInitialized() const { return initialized; }
...
int EspUart::send(const uint8_t *data, size_t length) {
  ...
  if (written > 0) {
    ...
    uint32_t timeout_ms =
        (length * 10 * 1000) / baudRate + 100; // Add 100ms buffer
    if (timeout_ms < 100)
      timeout_ms = 100; // Minimum 100ms
    if (timeout_ms > 5000)
      timeout_ms = 5000; // Maximum 5 seconds

    esp_err_t wait_result =
        uart_wait_tx_done(this->portNumber, pdMS_TO_TICKS(timeout_ms));
...
int EspUart::send(const std::string &data) {
  ...
  if (written > 0) {
    ...
    // Use a timeout based on the data size and baud rate
    uint32_t timeout_ms =
        (data.size() * 10 * 1000) / baudRate + 100; // Add 100ms buffer
    if (timeout_ms < 100)
      timeout_ms = 100; // Minimum 100ms
    if (timeout_ms > 5000)
      timeout_ms = 5000; // Maximum 5 seconds
...
int EspUart::receive(uint8_t *buffer, size_t maxLength) {
  ...
  int read = uart_read_bytes(this->portNumber, buffer, maxLength,
