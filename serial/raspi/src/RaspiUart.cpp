#include "RaspiUart.h"
#include "../../pUtils/raspi/include/raspilogger.h"
#include <cstring>
#include <errno.h>
#include <fcntl.h>
#include <iostream>
...
RaspiUart::RaspiUart(const std::string portName, const int portNumber)
    : fileDescriptor(-1), portName(portName), uartNumber(portNumber),
      initialized(false) {
...
bool RaspiUart::initialize(const SerialConfig &config,
  ...
  if (fileDescriptor < 0) {
    std::cout << "Error opening file descriptor for " << portName << ": "
  ...
  if (tcgetattr(fileDescriptor, &originalTios) != 0) {
    std::cout << "Error getting current terminal settings: " << strerror(errno)
              << std::endl;
...
int RaspiUart::send(const uint8_t *data, size_t length) {
  ...
  while (bytesWritten < length) {
    ssize_t written =
        write(fileDescriptor, data + bytesWritten, length - bytesWritten);
...
int RaspiUart::send(const std::string &data) {
  ...
  while (bytesWritten < totalBytes) {
    ssize_t written =
        write(fileDescriptor, buffer + bytesWritten, totalBytes - bytesWritten);
...
int RaspiUart::receive(std::string &buffer, size_t maxLength) {
  ...
  while (totalRead < static_cast<ssize_t>(maxLength)) {
    dataRead =
        read(fileDescriptor, charBuffer + totalRead, maxLength - totalRead);
...
bool RaspiUart::isOpen() const { return fileDescriptor >= 0; }
...
bool RaspiUart::configureTermios() {
  ...
  case 5:
    currentTios.c_cflag |= CS5;
    break;
  case 6:
    currentTios.c_cflag |= CS6;
    break;
  case 7:
    currentTios.c_cflag |= CS7;
    break;
  case 8:
    currentTios.c_cflag |= CS8;
    break;
  default:
    currentTios.c_cflag |= CS8;
    break;
  ...
  uint8_t timeout_deciseconds =
      (config.timeoutMs + 99) / 100; // Round up division
  if (timeout_deciseconds < 1)
  ...
  if (tcsetattr(fileDescriptor, TCSANOW, &currentTios) != 0) {
    std::cout << "Error applying terminal settings: " << strerror(errno)
              << std::endl;
    return false;
  }
